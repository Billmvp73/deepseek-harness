...
Six full-suite test files fail on this host and none is merge-caused: every file the merge changed lies outside their import closures, so their inputs are byte-identical to upstream `c291e7961a`. [`real-product.spec.ts`](../../../../packages/subagent/subagent-claude-code/tests/real-product.spec.ts) needs the real Claude Agent SDK and times out without it. [`spawn-runner.spec.ts`](../../../../packages/subprocess/subprocess-local/tests/spawn-runner.spec.ts) asserts a Windows PATH probe order this Linux host does not reproduce. [`subagent-acp.spec.ts`](../../../../packages/subagent/subagent-acp/tests/subagent-acp.spec.ts) fails its teardown ladder because this host's systemd rejects `SIGKILL` to a scope's auxiliary processes with `Invalid argument`. [`runtime.spec.ts`](../../../../packages/experimental/code-runtime-python/tests/runtime.spec.ts) hits its 60-second wall-clock ceiling only under full-suite load and passes in isolation. Nine tests in [`spill-local.spec.ts`](../../../../packages/spill/spill-local/tests/spill-local.spec.ts) and [`loader-composition.spec.ts`](../../../../packages/spill/spill-local/tests/loader-composition.spec.ts) observe a startup cleanup sweep that deletes nothing and reports no warning; the cause is unidentified and belongs to that package, not to this sync.
