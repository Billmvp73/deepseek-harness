...
export interface SessionPromptRequest {
  ...
  /**
   * Request-local start-in-a-new-worktree provenance (like `clientTimeZone`):
   * when true and the target session is still blank, the Host relocates into a
   * fresh git worktree and names the moved-to session via
   * {@link SessionPromptValue.sessionId}. Non-blank sessions ignore the flag.
   */
...
export interface SessionPromptValue {
  ...
  /** The relocated session id when `newWorktree` started a fresh worktree session. */
  readonly sessionId?: SessionId
}
