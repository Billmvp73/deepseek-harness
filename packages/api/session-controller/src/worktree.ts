/**
 * Git worktree creation behind `session.prompt`'s request-local `newWorktree`
 * flag: before a blank session's first turn runs, the Host relocates the work
 * into a fresh linked worktree of the repository owning the session cwd.
 * Detection and creation both shell out to the user's `git`; a cwd outside any
 * repository resolves to `null` so the caller can ignore the flag exactly like
 * an absent one.
 */
 ...
 * on a new branch at the current HEAD. The worktree directory is a sibling of
 * the repository: `<parent>/<repo>.worktrees/<stamp>`.
