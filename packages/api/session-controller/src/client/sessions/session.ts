...
export interface SessionOptions {
  ...
  /**
   * A prompt requested a new-worktree relocation and the Host started the
   * prompt in a DIFFERENT session (the response's `sessionId` differs from
   * this session's id). The manager selects/opens the moved-to session; this
   * blank source session is abandoned rather than flipped engaged.
   */
  onMoved?(session: Session, movedTo: SessionId): void
  /**
...
export class Session implements SessionFace {
   ...
   *   worktree; when it does, the response carries the moved-to `sessionId`
   *   and {@link SessionOptions.onMoved} fires instead of the engage edge.
  ...
  async prompt(
  ...
  ): Promise<RemoteResult<{ accepted: true; sessionId?: SessionId }>> {
    ...
    this.notifier.markDirty()
    let result: RemoteResult<{ accepted: true; sessionId?: SessionId }>
    if (this.address === undefined) {
      ...
      // The Host reads the durable descriptor for one-shot rejection and
      // attachment support; the wire marker stays 'continuable' and content
      // reaches the wire unfiltered.
    ...
    // A new-worktree relocation starts the prompt in a different session: this
    // source session stays blank (abandoned) and the caller follows the move.
    const movedTo = result.ok ? result.value.sessionId : undefined
    if (movedTo !== undefined && movedTo !== this.sessionId) {
      this.options.onMoved?.(this, movedTo)
      return result
    }
