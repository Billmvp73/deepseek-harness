...
`session.prompt` 还接受请求本地的 `newWorktree` 来源信息：在 cwd 位于某个 git 仓库内的仍为空白（blank）的会话上，Host 会在该仓库创建一个 linked worktree（`<parent>/<repo>.worktrees/wt-<timestamp>`，分支 `wt-<timestamp>` 指向 HEAD），在那里启动一个新会话（相同组合、复制模型选择），把这条提示投递给它，并通过响应中的 `sessionId` 返回新会话 id，供调用方转移选中项。worktree 是仓库的同级目录；工作区领域按"linked worktree"记账，因此新会话仍归属于**同一个**工作区（源工作区的 `addWorktree` 先注册该路径，再附挂会话），而不是新建一个侧边栏分组。Session header 不可变，因此迁移采用全新身份；源会话保持空白、可复用。cwd 不在任何 git 仓库内时忽略该标记并就地投递；非空白会话同样忽略（该标记是"在新 worktree 中开始"的意图，绝非会话中途迁移）。仓库检查通过之后 git 失败，或新会话／其工作区记账失败，会以 `worktree-failed`、`workspace-attach-failed` 或 `internal` 拒绝——没有任何内容到达模型。
