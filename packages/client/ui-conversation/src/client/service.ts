...
export class ConversationController extends Service implements IConversation {
  ...
  async sendSession(
    ...
    // A `newWorktree` relocation resolves the prompt in a NEW session; the
    // Session's own onMoved callback (SessionManager.mergeSummary + select)
    // follows it, so the caller only carries the flag here.
